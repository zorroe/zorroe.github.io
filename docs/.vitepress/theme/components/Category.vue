...
<template>
  <div v-if="headers.length > 0" class="category">
    <ul class="list">
      <li v-for="(item, idx) in headers" :key="idx" class="header">
        <a v-if="item.level === 2" :href="item.link" class="header-h2">{{ item.title }}</a>
        <ul v-if="item.level === 3">
          <li class="header">
            <a :href="item.link" class="header-h3" :class="[{ showIndent }]">{{ item.title }}</a>
          </li>
        </ul>
      </li>
    </ul>
  ...
  .category {
    ...
    box-shadow: 6px 6px var(--vp-c-brand-lighter);
  }
}
.category {
  width: 20rem;
